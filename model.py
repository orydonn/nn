...
def weighted_roc_auc(y_true_int, y_pred_proba, weights_dict, label_encoder_for_target):
...
print("Calculating start_cluster transition matrix for month_6 imputation...")
...
if train_start_cluster_mode is None and len(all_original_start_clusters_str) > 0:
    ...
    print(f"Warning: train_start_cluster_mode is None. Using '{train_start_cluster_mode}'.")
...
if not transition_data_frames:
    print("Warning: No data available to calculate transition matrix. Using simpler imputation for month_6.")
...
test_df['start_cluster'] = test_df['start_cluster'].astype(str).astype(start_cluster_dtype) 
print(f"Test df month_6 start_cluster NaNs after advanced imputation: {test_df[test_df['month_num'] == 6]['start_cluster'].isna().sum()}")
...
print("Extended feature engineering finished.")
...
print(f"Final Numerical Features ({len(numerical_features_final)}): {numerical_features_final[:10]}...") # Print more
print(f"Final Categorical Features ({len(categorical_features_final)}): {categorical_features_final[:5]}...")
...
if train_val_df.empty: exit("Error: train_val_df is empty after month filtering.")
if valid_df.empty: exit("Error: valid_df is empty after month filtering.")
...
if test_month_6_df.empty:
    print("Warning: test_month_6_df is empty. Predictions for test will be based on an empty structure.")
...
print(f"Raw X_train_val shape: {X_train_val_raw.shape}, y_train_val shape: {y_train_val_int.shape}")
print(f"Raw X_valid shape: {X_valid_raw.shape}, y_valid shape: {y_valid_int.shape}")
print(f"Raw X_test_month_6 shape: {X_test_month_6_raw.shape}, Test IDs shape: {ids_test_month_6.shape}")

if X_train_val_raw.empty or X_valid_raw.empty: exit("Error: Raw train_val or valid set is empty.")
if y_train_val_int.empty or y_valid_int.empty: exit("Error: Target for train or validation is empty.");
...
for col in categorical_features_final:
    ...
    print(f"  Processing categorical column '{col}': {n_unique_train} unique values in training split.")
...
print("pd.get_dummies applied.")

train_ohe_cols = X_train_val_cat_ohe.columns
print(f"Number of OHE columns from training data after potential capping: {len(train_ohe_cols)}")

print("Reindexing OHE columns for validation and test sets...")
...
print(f"X_train_val_final shape: {X_train_val_final.shape}")
print(f"X_valid_final shape: {X_valid_final.shape}")
print(f"X_test_m6_final shape: {X_test_m6_final.shape}")
...
print("Target variable one-hot encoded.")

if X_train_val_final.empty or X_valid_final.empty: exit("Error: Training or validation set is empty after NN preprocessing.");
if y_train_val_int.empty or y_valid_int.empty: exit("Error: Target for training or validation is empty.");
if X_train_val_final.shape[1] == 0 and len(X_train_val_final) > 0 : exit("Error: X_train_val_final has no columns but has rows!");
elif X_train_val_final.shape[1] == 0 and len(X_train_val_final) == 0: 
    print("Warning: X_train_val_final is completely empty.");
    if not (len(y_train_val_int) == 0 and len(y_train_val_ohe) == 0): exit("Error: X_train_val_final empty but y_train_val not.")
...
if X_train_val_final.shape[1] > 0: 
    ...
    print(f"NN Validation Weighted ROC AUC: {valid_roc_auc_nn}")
    ...
    if not X_test_m6_final.empty:
        ...
        print("Test set for month 6 is empty. Creating an empty submission structure or default predictions.")
    ...
    print("Error: No features available for training the model. Skipping model training and prediction.")
    submission_df_nn = pd.DataFrame(columns=['id'] + list(le_target.classes_))
...
for col in submission_cols_order:
    if col not in submission_df_nn.columns and col != 'id':
        print(f"Adding missing column {col} to submission_df_nn")
...
print(f"Submission shape: {submission_df_nn.shape}, Sample submission shape: {sample_submission_df.shape}")

if not ids_test_month_6.empty and submission_df_nn.shape[0] != len(ids_test_month_6): 
    print(f"Warning: Submission row count ({submission_df_nn.shape[0]}) mismatch with expected test IDs for month 6 ({len(ids_test_month_6)}).")
elif ids_test_month_6.empty and not submission_df_nn.empty :
     print(f"Warning: Test IDs for month 6 were empty, but submission is not. Submission rows: {submission_df_nn.shape[0]}")
...
if not all(c in submission_df_nn.columns for c in sample_submission_df.columns) or \
   not all(c in sample_submission_df.columns for c in submission_df_nn.columns):
    print(f"Warning: Submission columns mismatch sample.")
